...
items.Push({ name: "Google Antigravity", cmd: "https://aistudio.google.com" })
items.Push({ name: "Claude", cmd: "https://claude.ai" })
items.Push({ name: "Gemini", cmd: "https://gemini.google.com" })
...
BuildUI() {
    ...
    Loop items.Length {
        ...
        ; Button Image
        PicObj := LauncherGui.Add("Picture", "x" MARG " y" yButtons " w" bw " h" bh, "HBITMAP:" gHbmN)
...
Launcher_SetState(idx, state) {
    ...
    ; In v2, updating picture control needs proper syntax
    ; Use HBITMAP:* to force a copy, preventing AHK taking ownership of our global handle
    try btnMap[idx].Pic.Value := "HBITMAP:*" hbm
}
...
Launcher_GdipStart() {
    ...
    NumPut("UInt", 1, si, 0)
    DllCall("gdiplus\GdiplusStartup", "Ptr*", &gToken, "Ptr", si.Ptr, "Ptr", 0)
}
